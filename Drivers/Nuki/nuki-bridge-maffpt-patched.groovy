...
@Field static _nukiBridgeDriverVersion = "0.9.0-ben1"           // Current version of this driver
...
metadata 
{
    definition (name: "Nuki Bridge", namespace: "maffpt.nuki", author: "Marco Felicio") 
    {
        ...
        // Local additions (benberlin, 2026-09-18): recover from a bridge IP change without re-adding the bridge
        command "testBridge"
        command "setBridgeAddress", [[name: "ip*", type: "STRING", description: "Bridge IP address"],
                                     [name: "token", type: "STRING", description: "Optional new API token"]]
        command "registerCallbacks"
...
//=========================================
// Local additions: bridge address recovery
//=========================================

// Queries /info with the stored IP and token; the outcome is logged at info/warn level
// regardless of the logging preferences so it can be used as a connectivity test.
def testBridge ()
{
    def uriString = "${buildBridgeURL (device.data)}/info?token=${device.data.Token}"
    try
    {
        httpGet ([uri: uriString, contentType: "application/json", timeout: _nukiHttpRequestTimeout])
        {
            resp ->
                def v = resp.data?.versions
                log.info "${device.label}: bridge at ${device.data.IP} OK - firmware ${v?.firmwareVersion}, wifi ${v?.wifiFirmwareVersion}, ${resp.data?.scanResults?.size()} paired device(s), uptime ${resp.data?.uptime}s"
        }
    }
    catch (e)
    {
        log.warn "${device.label}: bridge at ${device.data.IP} did NOT answer: ${e.message}"
    }
}

// Re-points this bridge device at a new IP (and optionally a new token) after a DHCP change,
// re-registers the callbacks and runs testBridge. Child lock/opener devices are untouched.
def setBridgeAddress (String ip, String token = null)
{
    ip = ip?.trim ()
    if (!ip) { log.warn "${device.label}: setBridgeAddress - no IP given"; return }

    device.updateDataValue ("IP", ip)
    if (token?.trim ()) device.updateDataValue ("Token", token.trim ())

    // Bridge callbacks are routed to this device by the IP-in-hex DNI
    def dni = parent.getIPHex (ip)
    if (device.deviceNetworkId != dni) device.deviceNetworkId = dni
    log.info "${device.label}: bridge address set to ${ip} (DNI ${dni})"

    registerCallbacks ()
    testBridge ()
}

// (Re)registers this hub as callback target on the bridge using the stored IP/token
def registerCallbacks ()
{
    def d = [IP: device.data.IP, Port: device.data.Port, Token: device.data.Token, Label: device.data.Label]
    try
    {
        setupCallbacks (d)
        log.info "${device.label}: callbacks registered on bridge ${d.IP}"
    }
    catch (e)
    {
        log.warn "${device.label}: registering callbacks on ${d.IP} failed: ${e.message}"
    }
}

// Frees this device's IP-hex DNI (for a duplicate bridge device that is about to be removed)
def releaseAddress ()
{
    def dni = "NukiBridge-dup-${device.id}"
    device.deviceNetworkId = dni
    log.info "${device.label}: DNI changed to ${dni}; this device no longer receives bridge callbacks"
}
