...
def parse (Map jsonMap)
{
    ...
    parent.sendBatteryEvent (device, batteryLevelFrom (jsonMap))
...
def status ()
{
    ...
    if (deviceInfo) parent.sendBatteryEvent (device, batteryLevelFrom (deviceInfo))
...
//
// Local fix (benberlin, 2026-09-19): the bridge reports no charge percentage for the Opener, only a
// batteryCritical flag. Upstream passed that boolean straight into the "battery" attribute ("false").
// Use the percentage when a newer bridge firmware supplies one, otherwise map the flag to a level.
//
def batteryLevelFrom (info)
{
    if (info?.batteryChargeState != null) return info.batteryChargeState
    return info?.batteryCritical ? 10 : 100
}
